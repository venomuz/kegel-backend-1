...
migration-up:
	migrate -path ./internal/migration -database 'mysql://kilogram:112233@tcp(localhost:3306)/crm?multiStatements=true' up

migration-down:
	migrate -path ./internal/migration -database 'mysql://kilogram:112233@tcp(localhost:3306)/crm?multiStatements=true' down

migration-gen:
	bash scripts/migrator.sh